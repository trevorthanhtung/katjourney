...
export function ChatBox({ token, currentUser, onClose, inline, isReadOnly = false }: ChatBoxProps) {
  ...
  const renderSenderAvatar = (senderName: string) => (
    <div className="w-full h-full">{getAvatarSvg(senderName)}</div>
  );

  const renderRoleIcon = (role?: string) => {
    if (!role) return null;
    const isLead =
      role.toLowerCase().includes("trưởng") || role.toLowerCase().includes("người tạo");
    if (isLead) {
      return (
        <span
          className="inline-flex items-center justify-center ml-1.5 select-none"
    ...
    return (
      <span
        className="inline-flex items-center justify-center ml-1.5 select-none opacity-80"
        title="Thành viên"
      >
        ...
        <div className="bg-white/60 dark:bg-[#0A0F1C]/40 backdrop-blur-xl p-4 text-kat-dark dark:text-white flex justify-between items-center shrink-0 border-b border-slate-200/60 dark:border-white/10 shadow-sm z-10">
          <div className="flex items-center gap-3 min-w-0">
            {isMobileFullscreen && onBackClick && (
              ...
              >
                <HugeiconsIcon
                  ...
                  className="w-5 h-5 text-kat-dark dark:text-white"
                />
              </button>
            )}
            <div className="w-10 h-10 rounded-full bg-[#00BFB7]/10 flex items-center justify-center border border-[#00BFB7]/20 shadow-inner shrink-0">
              <HugeiconsIcon
                icon={BubbleChatIcon}
                className="w-5 h-5 text-[#00BFB7] animate-pulse"
              />
            </div>
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5 text-[11px] text-slate-500 dark:text-slate-400 truncate">
                <span className="truncate">
                  {t("chat.chattingAs")}{" "}
                  <span className="font-semibold text-kat-dark dark:text-slate-200">
                    {currentUser.name}
                  </span>
                </span>
              </div>
          ...
          {loading ? (
            ...
            messages.map((msg, index) => {
              ...
              const showDateSeparator =
                index === 0 || getMessageDateString(messages[index - 1].createdAt) !== dateStr;

              const prevMsg = index > 0 ? messages[index - 1] : null;
              const nextMsg = index < messages.length - 1 ? messages[index + 1] : null;

              const isPrevSame =
                prevMsg && prevMsg.senderName === msg.senderName && !showDateSeparator;

              const nextDateStr = nextMsg ? getMessageDateString(nextMsg.createdAt) : "";
              const showNextDateSeparator = nextMsg && nextDateStr !== dateStr;
              const isNextSame =
                nextMsg && nextMsg.senderName === msg.senderName && !showNextDateSeparator;
              ...
              return (
                <React.Fragment key={msg.id || index}>
                  ...
                  >
                    ...
                    <div
                      className={classNames("flex flex-col", isMe ? "items-end" : "items-start")}
                    >
                      {!isMe && showAvatar && (
        ...
        <div className="w-full">
          ...
          <div className="block sm:hidden bg-white dark:bg-kat-surface border border-kat-border dark:border-kat-border/40 rounded-[24px] p-6 shadow-soft dark:shadow-none text-center flex flex-col items-center justify-center h-[280px]">
            <div className="w-16 h-16 rounded-full bg-kat-primary-soft flex items-center justify-center mb-4 relative">
              <HugeiconsIcon
                icon={BubbleChatIcon}
                className="w-7 h-7 text-kat-primary animate-pulse"
              />
            ...
            <p className="text-xs text-kat-muted mt-1 mb-5 leading-relaxed">
              {t("chat.connectedAs")}{" "}
              <strong className="text-kat-text font-semibold">{currentUser.name}</strong>
            </p>
            <button
