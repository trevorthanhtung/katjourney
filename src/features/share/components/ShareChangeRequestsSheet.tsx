...
import { approveChangeRequest, rejectChangeRequest } from "../../../services/cloudShareService";
...
function formatRequestTime(createdAt: any) {
  if (!createdAt) return i18n.t("common.justNow", "Just now");
  try {
    ...
    if (typeof createdAt === "object" && createdAt.seconds !== undefined) {
      return new Date(
        createdAt.seconds * 1000 + Math.floor((createdAt.nanoseconds || 0) / 1000000)
      ).toLocaleString();
    }
...
export function ShareChangeRequestsSheet({
  isOpen,
  onClose,
  token,
  requests,
  ...
  useModalHistory(
    isConfirmApproveAllOpen,
    () => setIsConfirmApproveAllOpen(false),
    "approve-all-confirm"
  );
  useModalHistory(
    isConfirmRejectAllOpen,
  ...
  function renderChangeDetails(req: AppChangeRequest) {
    ...
    const formatObj = (o: any) => {
      ...
      return t("share.itemUpdated", "Item details");
    };
    ...
    if (section === "members" && action === "update") {
      ...
      return (
        <div className="mt-2 space-y-1">
          <p className="text-[13px] font-bold text-slate-700 dark:text-slate-400">
            {t("share.sectionLabelRole", { sectionName })}
          </p>
          <div className="text-[13px] mt-1.5 font-medium text-slate-800 dark:text-slate-200">
            {t("share.changeRoleFor", "Xin đổi vai trò cho")}
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700/50">
              <span className="text-slate-400 dark:text-slate-500 line-through shrink-0">
                {oldRole}
              </span>
  ...
  return (
    <>
      <BottomSheet
        ...
        subtitle={
          requests.length > 0
            ? t("share.pendingRequests", { count: requests.length })
            : t("share.noRequests")
        }
        footer={
          requests.length > 1 ? (
          ...
          {requests.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-[14px] font-bold text-slate-500">
                {t("share.noRequestsDetailed")}
              </p>
            </div>
          ) : (
            requests.map((req) => {
              let requesterMember = members.find(
                (m) =>
              ...
              return (
                <div
                  key={req.id}
                  className="bg-white/60 dark:bg-[#0A0F1C]/40 backdrop-blur-xl rounded-[20px] border border-slate-200/60 dark:border-white/5 p-4 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] transition-all"
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[14px] font-bold text-slate-800 dark:text-slate-100">
                          {req.requesterName || t("sharedScreen.sharedUser")}
                        </p>
                  ...
                  <div className="mt-3.5">{renderChangeDetails(req)}</div>
