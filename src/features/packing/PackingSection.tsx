...
export function PackingSection({
  ...
  const [tripType, setTripType] = useState<PackingTripType>("beach");
  ...
  const stats = getChecklistStats(
    packingItems.map((item) => ({ ...item, section: "Before Trip" as const }))
  );
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="space-y-6 pb-36 md:pb-8">
        ...
        <section>
          {packingItems.length ? (
            ...
            <EmptyCard
              text={t("packing.emptyStateDetailed")}
              icon={<HugeiconsIcon icon={Luggage01Icon} className="h-6 w-6" />}
            />
          )}
        </section>
      </div>
      ...
      <BottomSheet
        ...
        title={editing ? t("packing.editItem") : t("packing.addItem")}
      >
        <div className="space-y-4">
          <Input
            label={t("packing.itemNameLabel")}
            value={title}
            onChange={setTitle}
          ...
          <div className="pt-2">
            <FormActions
              onSave={() => void addOrEditItem()}
              saveLabel={editing ? t("packing.saveChanges") : t("packing.addToList")}
            />
          </div>
        </div>
      </BottomSheet>
