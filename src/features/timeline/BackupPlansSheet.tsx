...
const typeColors: Record<BackupPlanType, string> = {
  food: "text-white bg-amber-500 border-amber-500 shadow-[0_4px_12px_rgba(245,158,11,0.3)]",
  place: "text-white bg-[#00BFB7] border-[#00BFB7] shadow-[0_4px_12px_rgba(0,191,183,0.3)]",
  transport: "text-white bg-blue-500 border-blue-500 shadow-[0_4px_12px_rgba(59,130,246,0.3)]",
  hotel: "text-white bg-violet-500 border-violet-500 shadow-[0_4px_12px_rgba(139,92,246,0.3)]",
  indoor: "text-white bg-emerald-500 border-emerald-500 shadow-[0_4px_12px_rgba(16,185,129,0.3)]",
  weather: "text-white bg-rose-500 border-rose-500 shadow-[0_4px_12px_rgba(244,63,94,0.3)]",
  other:
    "text-white bg-slate-700 dark:bg-slate-600 border-slate-700 dark:border-slate-600 shadow-[0_4px_12px_rgba(51,65,85,0.3)]",
};

export function BackupPlansSheet({
  tripId,
  activityId,
  date,
  isOpen,
  ...
  const plans =
    useLiveQuery(() => {
      ...
      return query
        .toArray()
        .then((all) =>
          all.filter((p) => !p.isDeleted && p.activityId === activityId && p.date === date)
        );
              ...
              <div className="border-t border-slate-200 dark:border-kat-border pt-4 mt-2">
                ...
                {showAdditionalInfo && (
                  <div className="space-y-4 mt-4 animate-fadeIn">
                    ...
                    <div>
                      <label className="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-1.5 flex items-center gap-1.5">
                        <HugeiconsIcon icon={DollarSignIcon} className="w-4 h-4 text-slate-400" />{" "}
                        {t("backup.estimatedCost")}
                      </label>
                      <input
                        type="number"
                        value={estimatedCost}
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                {plans.map((plan) => (
                  <div
                    key={plan.id}
                    className="p-4 rounded-2xl bg-white dark:bg-slate-800/30 border border-slate-200 dark:border-kat-border hover:border-[#00BFB7]/30 dark:hover:border-[#00BFB7]/50 transition-all"
                  >
                      ...
                      <div>
                        ...
                        {plan.reason && (
                          <div className="mt-2.5 text-[13px] font-bold text-amber-700 dark:text-amber-400 bg-amber-50/50 dark:bg-amber-950/15 border border-amber-100/60 dark:border-amber-900/30 px-3 py-1.5 rounded-xl leading-relaxed">
                            <span className="text-amber-600 dark:text-amber-500">Khi:</span>{" "}
                            {plan.reason}
                          </div>
                        )}
