...
import { Airplane01Icon, ChevronRightIcon } from "@hugeicons/core-free-icons";
...
export function ImportTripSheet({ isOpen, onClose, showToast }: ImportTripSheetProps) {
  ...
  const [recentSharedTrips, setRecentSharedTrips] = useState<
    { token: string; title: string; date: string; timestamp: number }[]
  >([]);
      ...
      <div className="space-y-6">
        <div className="relative bg-white/60 dark:bg-[#0A0F1C]/60 backdrop-blur-2xl border border-slate-200/60 dark:border-white/10 rounded-[28px] p-5 shadow-xl dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden animate-slideUpFade">
          ...
          <div className="relative z-10 space-y-3">
            <label
              htmlFor="shared-link-input"
              className="text-[13px] font-bold text-slate-500 dark:text-slate-400 block uppercase tracking-wider"
            >
                ...
                >
                  ...
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-50 dark:bg-slate-900/50 group-hover:bg-kat-primary/10 transition-colors duration-300 shrink-0">
                    <HugeiconsIcon
                      ...
                      className="h-5 w-5 text-slate-400 dark:text-slate-500 group-hover:text-kat-primary dark:group-hover:text-cyan-400 group-hover:translate-x-0.5 transition-all duration-300"
                    />
                  </div>
                </div>
              ))}
