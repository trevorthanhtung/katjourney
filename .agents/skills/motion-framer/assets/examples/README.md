...
function AnimatedCounter({ from = 0, to = 100 }) {
  ...
  return (
    <div ref={ref} style={{ minHeight: "50vh", display: "flex", alignItems: "center" }}>
      <motion.span style={{ fontSize: "4rem", fontWeight: "bold" }}>{smoothCount}</motion.span>
    </div>
  );
}
```
...
function ViewToggle() {
  ...
  return (
    <div>
      <button onClick={() => setIsGrid(!isGrid)}>Toggle {isGrid ? "List" : "Grid"} View</button>
...
function MegaMenu() {
  ...
  return (
    <nav style={{ background: "white", padding: "16px 0", borderBottom: "1px solid #e5e7eb" }}>
      <div
        style={{ display: "flex", gap: 32, maxWidth: 1200, margin: "0 auto", padding: "0 20px" }}
      >
